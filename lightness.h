...
extern "C" {
    LIGHTNESS_EXPORT QByteArray createPicture(unsigned char * imageData, int fileSize, int imageWidth, int imageHeight);
    LIGHTNESS_EXPORT float Min( float fR, float fG, float fB );
    LIGHTNESS_EXPORT float Max( float fR, float fG, float fB);
    LIGHTNESS_EXPORT void HLSToRGB( unsigned char &R, unsigned char &G, unsigned char &B, double H, double S, double L);
    LIGHTNESS_EXPORT void RGBToHSL( int R, int G, int B, double& H, double& S, double& L);
    LIGHTNESS_EXPORT void filter(unsigned char * image, unsigned char * result,  int width, int height, float bright);
}
